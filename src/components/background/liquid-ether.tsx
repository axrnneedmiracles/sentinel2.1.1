...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      update() {
        if(this.clock) {
          this.clock.getDelta(); // Not using delta, but must call to keep clock moving
        }
      }
    }
    ...
    class MouseClass {
      ...
      init(container: HTMLElement) {
        this.container = container;
        // Listen on window to catch mouse events even outside the container
      ...
      onDocumentMouseMove(event: MouseEvent) {
        ...
        if (!isInside) {
          if(this.isHoverInside) this.onMouseLeave();
          return;
        }
        if (!this.isHoverInside) this.onMouseEnter();
  ...
  return (
    <div
      ref={mountRef}
      className={`w-full h-full absolute inset-0 overflow-hidden touch-none ${className || ''}`}
